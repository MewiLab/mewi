...
class MicrologService:
    ...
    def create(self, body: MicrologCreate) -> MicrologRead:
        ...
        If embedding fails, the microlog is still saved without a vector.
        MicrologWorker will retry embedding via process_unembedded().
        """
        try:
            vector = self._embedder.embed_text(body.content)
        except EmbeddingError as exc:
            logger.warning("Embedding failed, saving microlog without vector: %s", exc)
            vector = []
