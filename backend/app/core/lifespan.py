...
async def lifespan(app: FastAPI):
    ...
    # ── Minimal mode: skip all external services ──────────────────────────────
    # Enable with MINIMAL_MODE=true. Only /health and /api/v1/intake will work.
    if settings.minimal_mode:
        logger.info(
            "MINIMAL MODE enabled — skipping Supabase, Redis, agent, and workers. "
            "Only /health and /api/v1/intake are active."
        )
        yield
        logger.info("Shutdown complete (minimal mode).")
        return
